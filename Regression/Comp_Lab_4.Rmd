...
The example is the example discussed in Demonstration CLass 3 and comes from the Doing Economics Project 3.
...
Execute the following command and interpret its outcome.
...
```{r, eval = FALSE}
tally(~XXXX+XXXX, data=XXXX, margins = TRUE, format = "count")
```
...
You should confirm that you are lef with 939 observations.
...
```{r, echo = FALSE}
mod1 <- plm(price_per_oz ~ time + did + sp_id, data = p_dat_c_ls, model = "fd")
stargazer_HC(mod1)
```
