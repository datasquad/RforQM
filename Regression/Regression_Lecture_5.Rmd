...
In this lecture we will use the firmsize variable `mfsize9`. When we import the data from the STATA dta file, the variables inherited certain attributes. YOu can see them if you look at the structure of the dataframe (`str(data_USoc)`). One of the attributes is the label and it contains a little bit of information on the variable.
...
You can see that there are 9 possible values (which is where the 9 in the name comes from). Each respondent who is in work works in a firm and the size of the firm is recorded, but we do not observe the actual size. There are buckets for firm sizes, e.g. 51 to 100, 101 to 200, 201 to 500, etc. And the number we see is the mid-value of the respective bucket into which a firm falls.
...
```
Looking again at `str(data_USoc$fsize)` we confirm that this is now a factor variable.
...
You can see that the average pay increases with firm size and that the variation is clearly largest in the 1-person companies.
...
You can see that all firm size categories have been included as dummy variables but for the 1-person firm. This is the base category and the estimated constant term is the average value for `lnhrpay` in that base category (compare to the above table). The other coefficients are the difference in mean pay relative to the base category. As you can see the average pay in all but one of the other categories is larger than in the 1-person firms.

The 1-person firm category was the base category as R orders factors alphabetically/numerically and "1" comes before all of the other category labels. You may have reasons to want to re-estimate the model with a different base category. Say you want the largest category to be the base category. If we want to do that we need to change the ordering of our levels in the factor variable `fsize`. (I had to google "r factor change level order" to remind myself of how to do that.)
...
If we now re-estimate the above model we obtain different parameters, but in essence we are still replicating the same model and indeed the group means from the earlier table.
...
stargazer_HC(mod3)
```
In the absence of a constant each coefficient represents the sample mean of the respective firm size categories.

All the above specifications essentially estimate the same model. In particular we allow each of the firm size categories to have impacts which are independent of the other categories, which means that we just recover the respective sample means.
