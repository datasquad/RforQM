...
\begin{itemize}
  \item Reinhard and Rogoff seem to suggest that there is a level of debt (debt/GDP > 90\%) beyond which higher debt levels will significantly reduce growth.
...
\begin{tabular}{cl|}
	\hline 
	The New Yorker &  \href{https://www.newyorker.com/news/john-cassidy/the-reinhart-and-rogoff-controversy-a-summing-up}{The Reinhart and Rogoff Controversy: A Summing Up}\\ 
...
\begin{itemize}
  ...
  \item But in thepolitical discourse such "subtleties" often go lost
  \item Would different summary stats have changed the narrative?
\end{itemize}
...
\begin{itemize}
  ...
  \item Use this example to 
    \begin{itemize}
      ...
      \item review some summary satistics
...
Replication of work is actually at the core of the Reinhard/Rogoff controversy!

## Prepare your code

We start by uploading the extra packages we need in our code.

The first time you need these packaes at a computer you may need to install these. Use the following code to do this
...
and check some charecteristics of the data which are now stored in \texttt{RRData}:
...
\begin{itemize}
  ...
  \item \texttt{complex: 1+4i (complex numbers with real and imaginary parts)}
\end{itemize}

It is important that you know what data types variables have as some operations only work for particular datatypes. For instance we need to \texttt{num} or \texttt{int} for any mathematical operations.
In our data.frame three variables ar e\texttt{num} and one is of \texttt{chr} type. 
\texttt{logical} variables we will encounter frequently, they are very powerful.

## \texttt{factor} variables

It will prove useful to change variables which are categorical variables (here \texttt{Country}) to \texttt{factor} variables.
...
```{r, echo = TRUE, error = FALSE, message = FALSE, results = TRUE, warning = FALSE}
summary(RRData$Year)    # Year is the year of observation
summary(RRData$debtgdp) # the debt to GDP ratio
summary(RRData$dRGDP)   # the GDP growth rate
```
...
\vskip -0.5cm
\[rGDP_{it} = \alpha + \beta debtgdp_{it} + u_{it} ~\text{(\textcolor{student}{Population}~~~~ Model)}\]
\vskip -0.2cm

\begin{itemize}
  \item Here we have two subscripts as the data have a cross-section (\texttt{\textcolor{student}{i}}) and a time-series dimension (\texttt{\textcolor{student}{t}}).
  \item The population model is defined by unknown parameters $\alpha$ and $\beta$ and the unknown error terms $u_{it}$. We will use sample data to obtain sample estimates of these parameters.
  \item The error terms $u_{it}$ contain the effects of any omitted variables and reflect that any modelled relationship will only be an approximation. The $u_{it}$s are \textcolor{student}{random variables}
\end{itemize}

\vskip -0.5cm
\[ rGDP_{it} = \widehat{\alpha} + \widehat{\beta} debtgdp_{it} + \widehat{u}_{it} ~\text{(\textcolor{student}{Sample} ~~~~Model)}\]
\vskip -0.2cm
...
Regression analysis is the core technique used in Econometrics. It is based on certain assumptions about the \emph{Population Model} and the error terms $u_{it}$ (more on this in the next few weeks).

How to estimate parameters (get $\widehat{\alpha}$ and $\widehat{\beta}$) using the available sample of data?
...
\begin{eqnarray*}
  \widehat{\beta}&=&\dfrac{Cov(dRGDP_{it},debtgdp_{it})}{Var(debtgdp_{it})}\\
...
\textcolor{student}{An increase of one unit in \texttt{debtgdp} (=1\%) is related to a decrease of GDP growth of 0.018 units (=0.018\%)}
...
We need to recognise that paramter estimator is a random variable.

Let's use $y_{it}=\alpha + \beta x_{it} + u_{it}$ for ease of notation:
\vskip -0.5cm
\begin{eqnarray*}
  \widehat{\beta}&=&\dfrac{Cov(y_{it},x_{it})}{Var(x_{it})}\\
   &=&\dfrac{~~Cov({\alpha} + {\beta} x_{it} + {u}_{it}~,x_{it})}{Var(x_{it})}\\
   &=&\dfrac{~~Cov({\alpha},x_{it}) + Cov({\beta} x_{it},x_{it}) + Cov({u}_{it},x_{it})}{Var(x_{it})}\\
\end{eqnarray*}

\vskip -0.5cm

\begin{eqnarray*}
  \widehat{\beta}&=&\textcolor{student}{\dfrac{~~Cov({\alpha},x_{it})}{Var(x_{it})} + \beta\dfrac{Cov( x_{it},x_{it})}{Var(x_{it})} + \dfrac{Cov({u}_{it},x_{it})}{Var(x_{it})}}\\
   &=&\textcolor{student}{0 + \beta\dfrac{Var( x_{it})}{Var(x_{it})} + \dfrac{Cov({u}_{it},x_{it})}{Var(x_{it})}}\\
   &=&\textcolor{student}{ \beta + \dfrac{Cov({u}_{it},x_{it})}{Var(x_{it})}}
\end{eqnarray*}
...
\begin{itemize}
  ...
  \item For this to happen we need to \textcolor{red}{assume} that $Cov({u}_{it},x_{it})=0$ as then \\
...
\begin{itemize}
  ...
  \item Measurement error in $x_{it}$ (not dealt with here)
...
\[gr_{i}=\alpha + \beta ~att_{i} + u_{i}\]

What value would you expect $\widehat{\beta}$ to take?

\textcolor{student}{positive, more lecture attendance better grade}

Can we say that lecture attendances causes higher grades?
\textcolor{student}{What do we think is hidden in the error term? Intelligence, Attitude, Aptitude for quants. How are they related to $att_i$? Presumably some positively, e.g. attitude to studies. But all of tehse variables should really be in the model, right? So some of their effect is going to be capured by $att_i$ and this may bias the estimate $\widehat{\beta}$ up as it now captures, say, the effec of an increase in attendance but also in attitude. }
