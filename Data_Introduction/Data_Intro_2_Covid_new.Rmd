...
data <- read_csv(tf,col_types = "fffnfncnnc") # coltypes presets the types
```
...
```{r}
data %>%  filter(country == "Germany" & year_week == "2020-20")
```
...
```{r}
data <- data %>%  pivot_wider(names_from = indicator, values_from = c(weekly_count, cumulative_count,rate_14_day))
```

Check the number of observations in `data`. They will have halfed. Let's confirm the variable names we now have
...
```{r}
names(data)[names(data)=="weekly_count_cases"] <- "cases_weekly"
names(data)[names(data)=="weekly_count_deaths"] <- "deaths_weekly"
names(data)[names(data)=="cumulative_count_cases"] <- "cases_weekly_cumulative"
names(data)[names(data)=="cumulative_count_deaths"] <- "deaths_weekly_cumulative"
names(data)[names(data)=="rate_14_day_cases"] <- "cases_14_day"
names(data)[names(data)=="rate_14_day_deaths"] <- "deaths_14_day"
names(data)[names(data)=="year_week"] <- "dates"
names(data)[names(data)=="population"] <- "popData2019"
```
...
data<- merge(data,by = "geoID", countryInd,all.x=TRUE)
```
