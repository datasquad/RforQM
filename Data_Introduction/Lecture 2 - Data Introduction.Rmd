...
Is this different \textcolor{student}{~~~statistically and/or economically~~~} signifficant?
...
* Per-se it does \textcolor{red}{not deliver causal relationships}!
...
The estimate for the constant, $\widehat{\alpha}$, is merely the sample mean.
...
We will look at the interpretation of the estimate of $\widehat{\beta}$.
...
One of the regression assumptions is that the (unobserved) error terms $u_{i}$ are uncorrelated with the explanatory variable(s), here $Inc_{i}$. Then we call \textcolor{orange}{$Inc_i$ exogenous}.

This implies that $Cov(Inc_{i},u_{i})=Corr(Inc_{i},u_{i})=0$
...
In sample $Corr(Inc_{i},\widehat{u}_{i})=0$ ALWAYS.

## Regression Analysis - The Exogeneity Assumption

Why is **assuming** $Corr(Inc_{i},u_{i})=0$ important when, in sample, we are guaranteed $Corr(Inc_{i},\widehat{u}_{i})=0$?

If $Corr(Inc_{i},u_{i})=0$ is **not true**, then

1) Estimating the model by OLS \textcolor{student}{imposes a relationship which is not found in the data}
2) As a result, the estimated coefficients  \textcolor{blue}{$\widehat{\alpha}$ and $\widehat{\beta}$} are \textcolor{student}{biased (on average incorrect if we had many samples)}
...
Much of empirical econometric analysis is about making the exogeneity assumption more plausible/as plausible as possible.

1) Most models have reasonable more than one explanatory variable.
2) Including more relevant explanatory variables can make the exogeneity assumption more plausible.
3) Sometimes we need to apply some econometric/statistical tricks to make this assumption plausible, e.g. Instrumental Variable regression
