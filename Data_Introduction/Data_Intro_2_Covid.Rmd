...
NOTE 2: As of 19 February, the structure of that file which is downloaded has changed and hence will not work. You know that the dataset is not correct if you have more than 60,000 observations. If the file is incorrect, then you can use the data in StaticECDCdata_8Feb21.csv, which is a file with data up to 8 Feb 2021.
...
Extra note: The `geoID` variable for Namibia is missing. Let's add it here (it should be the two letter code "NA").

```{r}
levels(data$geoId) <- c(levels(data$geoId),"NA")
data$geoId[data$country == "Namibia"] <- "NA"
```
