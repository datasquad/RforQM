...
These are weekly data, but from the dataset alone it is not obvious whether these variables are cumulative (i.e. all the COVID-19 cases identified in a country up to that week), or only the cases identified in that particular week.  You could either go back to the data source to find an explanation or we could plot the data and use our understanding of what the data should look like. 

> Note: In the early days of the pandmic there was a lot of focus on daily statistics. However, it was quickly recognised that daily statistics exhibited a lot of predictable variation across the weekdays which made looking at the daily statistcs less useful. 
...
# specifies variable for horizontal and vertical axis, respectively
plot(data_china$dates,data_china$deaths_weekly)  
```

We can clearly see that after an increase in the numbers of fatalities in China in January and February we see a decrease in numbers in March. If these were cumulaive numbers we would not see a decrease. So, these must be the deaths which occured in a particular week. You can find the same for cases.
...
Before we apply this to our data in `data`, we need to make sure that we only accumulate separately by country (`group_by(country)`) and that the data are arranged by date (`arrange(dates)`).

```{r}
data <- data %>% group_by(country) %>% 
          arrange(dates) %>% 
          mutate(c_cases = cumsum(cases_weekly), c_deaths = cumsum(deaths_weekly)) %>% 
          ungroup()
```
...
First we replicate the above Figure but using the `ggplot` function, which produces much nicer charts. We will create the chart and save it as the object `g1`, then display it by just calling `g1`. Unlike in Section 1.3, we will use the subset function instead of creating a separate dataset containing only the data for China.

```{r}
g1 <- ggplot(subset(data, country == "China"), aes(x=dates,y=cases_weekly)) +
      geom_line() + 
