...
Import the data from the "StaticECDCdata_18Feb22.csv" file. Make sure the file is saved in your working directory, that you set the working directory correctly and that you set the the `na=` option in the `read.csv` function to the value in which missing values are coded in the csv file. To do this correctly you will have to open the csv file (with your spreadsheet software, e.g. Excel) and check for instance cell K2.
...
Thje dataset also contains data on entire continents but we will remove these datalies.
...
The date information is currently captured in the `dates` variable. Variable `dates` is currently a factor (`factor`) variable (originalla a `chr` but as we imported we converted these all to factor), but we want R to know that each string represents a date.
What we do here was described in detail in the code to Lecture 2.
...
```{r, eval = FALSE}
g1 <- XXXX(subset(data, country == "XXXX"), aes(x=dates,y=deaths_weekly)) +
      geom_XXXX() + 
...
From Table `temp` you should be able to identify that Pakistan and Nigeria are the next larger and next smaller country. Create a new variable `sel_countries` which you use to select data for these countries and then plot a line graph with weekly case numbers for these three countries. You may want to check out previous code (Week 2 Empirical Work Script) to see what needs doing.
...
As discussed in the Week 2 empirical workscript there is a tool just for that job, the `countrycode` library. We add a two digit country code to `data` and call it `geoID`.
...
Confirm that the average population density in your dataset is `r mean(data$popdens, na.rm=TRUE)`.
...
Here our $H_0$ is $H_0: \mu_{EU}=\mu_{AM}+100$ (or $\mu_{EU}-\mu_{AM}=50$) and we would test this against an alternative hypothesis of $H_A: \mu_{EU}>\mu_{AM}+100$ (or $H_A: \mu_{EU}-\mu_{AM}>100$). Here the statement of the politician is represented in the $H_A$. 
...
Let us estimate this again using the average rates calculated on data from June 2020 onwards only (hence the results here will be somewhat different to those in the lecture).
...
It is the p-value to that hypothesis test which is represented by the asteriks next to the estimated coefficient. Let's confirm that. The estimated coefficient to the `Obese` variable is 0.051 and the (***) indicate that the p-value to that test should be less than 0.01.
