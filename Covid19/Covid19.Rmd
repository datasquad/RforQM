...
For this example creating an animated map may be very instructive. The above maps were created for one particular day ("04/04/2020"). Could we create multiple maps for each day in 2020 so that we can visualise the spread of the pandemic. The answer is yes.
...
This updated data frame is no inserted back into the shape file.

```{r}
m3$tm_shape$shp <- temp3
```

Now we need to instruct `tmap` to produce the maps, but now a separate map for every day. The `tmap` command which is useful here is `tm_facets(along = "dates", free.coords = FALSE)`. The `along = "dates"` option selects the `dates` variable to the facet variable, i.e. the variable for which we calculate separate maps for every possible outcome (i.e. day).

```{r}
covid_anim <- m3 + tm_polygons(col = "deaths", n=10) + 
  tm_facets(along = "dates", free.coords = FALSE)
```

The result of this calculation was saved in `covid_anim`. In here we find al the information for all our maps. We are now left with producing a gif which connects the individual images. THIS DOES ONLY WORK IN A SCRIPT NOT FROM RMD.

```{r, eval = FALSE}
tmap_animation(covid_anim, filename = "covid_anim.gif", delay = 200)
```