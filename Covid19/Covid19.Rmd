...
You can see that the smoothed version (in blue) clearly shows an early exponential trend and the most recent decline (as of 7/4/2020). Change the values of the `span` option to figure out how it changes the results.
...
But smoothing can be useful. Here for instance we can perhaps see that France and Spain may have reached the peak of new registered cases, where, at the time of writing (7 April 2020), the UK numbers were still on a clearly positive trend.

In the example above the smoothing was done by and inside the `ggplot` function. You can also produce the moving average "manually" and then use it in teh forecast. This is useful if you need the moving average for some further analysis (as we will later). Here we will use the `forecast` package which you can use to calculate a moving average. Here we create a new variable `cases_ma = ma(cases,order = 7)` which calculates a 7 day moving average. By way of example, take the 10th of March. Instead of reporting the cases figure for the 10th of March we are now using 7 observations from 7th of March to 13th of March (i.e. centered around the 10th of March) and `cases_ma` will then have, for the 19th of March the average of these 7 days.
...
temp_mergein <- data %>% filter(dates %in% date_sel) %>% 
                  dplyr::select(geoId, dates, cases, c_cases, cases_ma, deaths, c_deaths, deaths_ma) %>% 
                  arrange(geoId,dates)
temp_mergein$geoId <- as.character(temp_mergein$geoId)
temp_mergein$geoId[temp_mergein$geoId == "UK"] <- "GB"  
```
...
Let's load some international travel data. We can get some data on flight connection information (from 2014, no later info is available) from the [https://openflights.org/data.html](Open Flights) website. Find the "routes.dat" datafile which is a csv file without column headings which is why we need to add the column names in the import process.
...
Let's explore the data by looking at all the routes connecting Manchester, UK, with Frankfurt, Germany.

We need to find the `IATA.ID` for Manchester and Frankfurt.
...
```{r}
data_routes %>% filter(Source.Airport == "MAN" & Destination.Airport == "CDG") %>% 
  dplyr::select(Airline, Source.Airport, Source.Country, Destination.Airport, Destination.Country)
```
...
Perhaps we can build a prediction tool of how the pandemic spreads. Basically we are building a transition matrix. We are using the moving average of daily cases (`cases_ma`) and combine them with the measure of strength of travel connection to think about where it is likely that the virus would spread. As we know, travel connection have very quickly been severed such that this way of anticipating the spread is likely to only work in the early stages of the pandemic.
...
Countries now contains 117 countries. These are 34 fewer countries than we have virus data for. It may well be that some of these are because countries in the two datasets have different spelling. TO check whether this is likely we will look at the set of 34 countries which have been excluded from the `countries_d` set.
...
First we will select all observations for the countries in `cou_list` and the dates in `date_sel`. Then we will organise the data such that the dates are variable nemes (using the `spread` function)
...
We proceed as follows. We take the data about current infections and multiply the number with the number of connections. We do this in a double loop. For every (receiving) country we will calculate the potential contribution of receiving the virus (cases in the source country * number of connections). All of these contributons are then summed up. The resulting number has no direct interpretation, but the larger the number the more likely that the virus will travel into that country
...
for (i in cou_list) { # loop for destimation country
  ...
  for (j in cou_list) { # loop for source country
    conn <- connections$connect[connections$Source.Country == j & connections$Destination.Country == i] 
    if (length(conn) == 0) {conn <- 0}   # ensure 0 if no connection
    supp <- spread[spread$country == j,as.character(date_sel[1])] * conn # pick current cases in source country (1st date in col 2)
    trans_fac <- trans_fac + supp
  }
  spread$sp_wk1[spread$country == i] <- unlist(trans_fac)  # need to unlist 
}

```

We will now plot the resulting information on a map
...
temp <- merge(temp, temp_mergein, by.x = "iso_a2", by.y = "geoId", all.x = TRUE)
m2$tm_shape$shp <- temp
m2 + tm_polygons(col = "sp_wk1", 
                 style = "fixed",
                 breaks = c(0,5,50,100,200,500,20000))  # n = 10 controls the number of categories
