...
table2 <- merge_data %>% filter(Mechanism == "Firearm") %>% 
                          group_by(State) %>% 
                          summarise(avg_aarate = mean(Age.Adjusted.Rate,na.rm = TRUE)) %>% 
...
```{r}
merge_data <- merge(merge_data,ur_data_long)
```

### Law enforcement officer numbers

The F.B.I. has a Crime Data Explorer and in there you can find a number of crime related data, amongst others the [Law Enforcement Employeies Data](https://cde.ucr.cjis.gov/LATEST/webapp/#) which provides a detailed breakdown of law enforcement employees for very small geographies like cities. As the datafile contains state information we can aggregate these to state level data.
...
As we need the number of officers in a state in one year we need to aggregate over all agencies in a particular year in a state. We also restrict our attention t oyears 2001 onwards.
...
plot1off <- officers_data_agg %>% 
                            filter(State_code %in% c("AL","TN","ME", "NY"))
...
```{r}
merge_data <- merge(merge_data,officers_data_agg)
```
...
```{r}
merge_data <- merge(merge_data,vcrime_data_long)
```
...
```{r}
merge_data <- merge(merge_data,alcc_data)
```
...
```{r}
merge_data <- merge(merge_data,incarceration_data)
```
