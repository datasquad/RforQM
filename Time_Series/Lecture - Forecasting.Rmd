...
\footnotesize
Look again at the forecast we produced earlier for 2019 and now also look at the forecast extending into 2020, using data up to December 2018.

Unemployment data for 2020 are not yet available, but clearly the above forecasts will substantially undershoot!
\tiny
```{r, echo = FALSE,  fig.height = 3.0, fig.width = 4.5}
for_ur_a <- forecast(fit_ur_a, h =24)
plot(for_ur_a, include = 20)
```
\normalsize

## Forecasting failure - Covid-19

On a previous slide we argued that minor forecats "failures"" will be due to random variation, failure to nclude all relevant variables, using the wrong model and parameter uncertainty.

Importantly, all forecasts are **contingent** on certain assumptions. Basicaly we need to assume that the model used is and **will stay** to be the correct model.

Therefore you often get **scenarios** rather than **forecasts**. For instance the [Office for Budget Responsibility's Covid-19 Scenario](https://obr.uk/coronavirus-reference-scenario/).

## Forecasting failure - Covid-19

The key scenario assumption here is 
...
If the above assumption is inappropriate, we will see changes in these modl features and significant deviations from any scenario predictions.
