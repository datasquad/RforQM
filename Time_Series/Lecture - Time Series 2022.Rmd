...
# keep all the data including 2021-Q4
...
-   here observations are from Q1 1955 to Q4 2021 (268 observations)
...
\begin{tabular}{ccc}
    ...
    253 & $y_{2021Q3}$ & $y_{2021Q2}$ \\

    254 & $y_{2021Q4}$ & $y_{2021Q3}$ \\     
    \hline 
\end{tabular}
```
...
-   this results in an ACF which has large $\rho_k$ for fairly large values of $k$ ($\rho_8 = 0.923$)
...
-   the ACF shows that the unemployment rate is also very persistent and the ACF only slowly converges to 0. $\Rightarrow$ a series can be persitent without time trend
...
-   If the ACF decays quickly to 0 then todays info is not very valuable for forecasting long into the future\
...
\hline 
Org & $\hat{\gamma}_1$ &  & 0.001 & 0.587*** & -0.134*** \\ 
& $se_{\hat{\gamma}_1}$ &  & (0.001) & (0.026) & (0.009) \\ 
& $R^2$  &  & 0.143  & 0.954 & 0.923  \\ \hline
Pass & $\hat{\gamma}_1$ & 159.463 &  & 106.734* & -15.061 \\ 
& $se_{\hat{\gamma}_1}$ &  (97.459) &  & (53.471) & (15.086) \\ 
& $R^2$ & 0.143  &  & 0.199 & 0.062 \\ \hline
Tert & $\hat{\gamma}_1$ & 1.644*** & 0.002* &  & -0.232*** \\ 
& $se_{\hat{\gamma}_1}$ & (0.074) & (0.001) &  & (0.014) \\ 
& $R^2$ &  0.965 & 0.199 &  & 0.938 \\ \hline
Alc & $\hat{\gamma}_1$ & -6.878*** & -0.004 & -4.043*** &  \\ 
& $se_{\hat{\gamma}_1}$ & (0.483) & (0.004) & (0.252) &  \\ 
& $R^2$ & 0.923 & 0.062  &0.938  &  \\ \hline
\end{tabular} \\
...
```{r}
mod_sr2 <- lm(Alc.Disease.FR~Organic.Farming.GE+index(data_sr), 
              data = data_sr)
```
...
```{r}
mod_sr3 <- lm(diff(Alc.Disease.FR)~diff(Organic.Farming.GE), 
              data = data_sr)
```
...
-   when regressing non-stationary series (in particular but not only when the series have a time-trend) will very likely deliver a spurious correlation\
...
For all these reasons, when considering time-series data, we need to **use stationary data. If not estimated coefficients will be, in general, neither unbiased nor consistent**.
...
\begin{table}[h] \centering 
    ...
    \begin{tabular}{@{\extracolsep{5pt}}lcc} 
        ...
        ADL (2 lags) &  6   & 635.2224      \\
        AR (2 lags) &   4   & 651.9734      \\
        ADL (4 lags) &  10 &    628.9836        \\
        AR (4 lags) &   6   & 644.1515\\
                \hline 
        \hline \\[-1.8ex] 
