...
Once you imported the data have a look at this data frame (`str(co2cdata)`) to understand what variables the data farme contains.
...
As we discussed before these data are nonstationary and it would be important to understand that this result is not merely a result of the non-stationary nature of the data. On this occasion it seems not really appropriate to merely incluse a time trend. It is the (potential) time-trend which is actually the oject of interest here. If we introduced it as an exogenous series we would potentially, und unjustifiably, remove the potential for rising co2 emissions explaining this trend.
